...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/admin_slots.css">
        ...
        <div class="dashboard-content">
            <div class="left-section">
                ...
                <div class="new-accounts-card">
                    <div class="info-panel">
                        ...
                        <div id="add-slot-container">
                            <div class="slot-accordion-container">
                                <div class="slot-accordion active">
                                    <div class="slot-accordion-header" onclick="toggleAccordion(this)">
                                        Add a Slot
                                    </div>
                                    <div class="slot-accordion-body active">
                                        <form id="slot-form">
                                            ...
                                            <div class="slot-form-group">
                                                <label>Parking Lot Location</label>
                                                <input type="text" name="locationName" list="locations" placeholder="Choose or enter new..." required>
                                                <datalist id="locations">
                                                </datalist>
                                            </div>
                                            <div class="slot-form-group">
                                                <label>Location Airport Code</label>
                                                <input type="text" name="location" placeholder="LAX" required>
                                            </div>
                                            <div class="slot-form-group">
                                                <label>Parking Lot Manager</label>
                                                <select name="lotManager" id="managers" required>
                                                </select>
                                            </div>
                                            <div class="slot-form-group">
                                            ...
                                            <div class="slot-form-group">
                                                <label>Charge</label>
                                                <input type="number" name="charge" step="0.01" required>
                                            </div>
                                            <div class="slot-form-group">
                                                <label>Overstay Charge</label>
                                                <input type="number" name="overstayCharge" step="0.01" required>
                                            </div>
                                            <button class="slot-save-btn" type="submit">Save Slot</button>
                                        </form>
                                    </div>
                                    <div class="slot-accordion">
                                        <div class="slot-accordion-header" onclick="toggleAccordion(this)">
                                            Add a Manager
                                        </div>
                                        <div class="slot-accordion-body">
                                            <form id="manager-form">
                                                <div class="slot-form-group">
                                                    <label>Manager</label>
                                                    <input type="text" name="managerName" list="locations" required>
                                                </div>
                                                <div class="slot-form-group">
                                                    <label>Commission (%)</label>
                                                    <input type="number" name="commission" step="0.01" required>
                                                </div>
                                                <button class="slot-save-btn" type="submit">Save Manager</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="right-section">
                <div class="search" id="search">
                    ...
                    <input type="text" id="searchInput" placeholder="search parking slots">
                </div>
                ...
                <!-- Template for slots entry -->
                <template id="slots-template">
                    <div class="slots-accordion">
                        ...
                        <div class="accordion-content">
                            <form class="user-form">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="location-input" disabled>
                                </div>
                                <div class="form-group">
                                    <label>Slot Name</label>
                                    <input type="text" class="slot-input" disabled>
                                </div>
                                <div class="form-group">
                                    <label>Available</label>
                                    <select class="available-input" disabled>
                                ...
                                <div class="form-group">
                                    <label>Charge</label>
                                    <input type="number" class="charge-input" disabled>
                                </div>
                                <div class="form-group">
                                    <label>Manager</label>
                                    <input type="text" class="manager-input" disabled>
                                </div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/assets/js/admin-slots.js"></script>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    function filterTransactions(filterType) {
        window.open('/print/transactions?filter=' + encodeURIComponent(filterType), '_blank');
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const firstAccordion = document.querySelector('.slot-accordion:first-child');
        firstAccordion.classList.add('active');
        firstAccordion.querySelector('.slot-accordion-body').classList.add('active');
    });

    function toggleAccordion(clickedHeader) {
        const accordions = document.querySelectorAll('.slot-accordion');
        const clickedAccordion = clickedHeader.closest('.slot-accordion');
        const wasActive = clickedAccordion.classList.contains('active');
        const otherAccordion = Array.from(accordions).find(acc => acc !== clickedAccordion);

        accordions.forEach(acc => {
            acc.classList.remove('active');
            acc.querySelector('.slot-accordion-body').classList.remove('active');
        });

        if (wasActive) {
            otherAccordion.classList.add('active');
            otherAccordion.querySelector('.slot-accordion-body').classList.add('active');
        } else {
            clickedAccordion.classList.add('active');
            clickedAccordion.querySelector('.slot-accordion-body').classList.add('active');
        }
    }
    ...
    document.getElementById("slot-form").addEventListener("submit", function (e) {
        ...
        const form = e.target;
        const formData = {
            lotType: form.lotType.value,
            location: form.location.value,
            locationName: form.locationName.value,
            lotManager: form.lotManager.value,
            slotName: form.slotName.value,
            charge: parseFloat(form.charge.value),
            overstayCharge: parseFloat(form.overstayCharge.value)
        };

        fetch('/post/slot/create', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    alert("Failed to save slot: " + (data.message || "Unknown error"));
                }
            })
            .catch(error => {
        ...
        fetch('/post/manager/add', {
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    alert("Failed to save manager: " + (data.message || "Unknown error"));
                }
            })
            .catch(error => {
